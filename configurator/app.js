...
const TABS = [
  ...
  { id: "headless", label: "HEADLESS", arch: "x86_64", fields: [
    ...
    { key: "headlessName", label: "Headless name", type: "text", def: "",
      help: "Container name + compose service id for the headless. Leave blank to use <server name>-headless.", re: /^[a-zA-Z0-9_.-]+$/ },
...
function emitCompose() {
  ...
  const hl = s.headlessName || `${svc}-headless`;   // blank = derive from the server name
