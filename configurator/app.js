...
const TABS = [
  ...
  { id: "ops", label: "OPS", fields: [
    ...
    { key: "quma", label: "Quartermaster (mod manager)", type: "toggle", def: false,
...
function emitCompose() {
  ...
  if (s.quma) {
    // Quartermaster (quma) — mounts the SAME data dir as the server and the Docker
    // socket. Starts as root, joins the socket's group, then drops to PUID:PGID
    // (no host-specific group_add needed). Self-bootstraps on first boot from .env.
    L.push(
      "",
